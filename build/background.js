function a(){
    // Always return false, ignoring the meta tag
    return false;
}

async function s(e){
    const[{result:t}] = await chrome.scripting.executeScript({
        target: {tabId: e},
        func: a
    });
    return t;
...
async function o(e){
    try {
        return await chrome.tabs.sendMessage(e, {currentState: !0});
    } catch {
        return;
    }
}
...
chrome.tabs.onActivated.addListener(async function(e){
    try {
        const {tabId: t} = e;
        if (!t) return chrome.action.setIcon({path: i});
        if (await s(t)) return chrome.action.setIcon({path: c});
        chrome.action.setIcon({path: await o(t) ? n : i});
    } catch {
        chrome.action.setIcon({path: c});
    }
});

chrome.webNavigation.onCompleted.addListener(async function(e){
    if (await s(e.tabId)) return chrome.action.setIcon({path: c});
...
chrome.runtime.onMessage.addListener(async(e, {tab: t}) => {
    if (e.close !== !0 || !(t != null && t.id)) return;
    await chrome.tabs.sendMessage(t.id, {toggle: !0});
